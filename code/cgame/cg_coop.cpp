...
void CG_CoopEnts_f( void )
{
	...
	for ( int i = 0; i < cg.snap->numEntities; i++ )
	{
		...
		else if ( es->eType == ET_PLAYER )
		{
			...
			name = va( "spec %i hp %i/%i pw 0x%x ef 0x%x force 0x%x shock %i push %i light amb %.0f %.0f %.0f dir %.0f %.0f %.0f", es->modelindex3, es->coopHealth, es->coopMaxHealth, es->powerups, es->eFlags,
