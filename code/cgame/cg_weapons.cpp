...
void CG_RegisterWeapon( int weaponNum ) {
	...
	case WP_SABER:
		...
		*/
		...
		// coop: white blade art (the mod's pk3), tinted through shaderRGBA for an exact RGB colour
		cgs.media.rgbSaberGlowShader		= cgi_R_RegisterShader( "gfx/jacoop/rgb_glow" );
		cgs.media.rgbSaberCoreShader		= cgi_R_RegisterShader( "gfx/jacoop/rgb_line" );
	...
	*/
	...
	{//lightsaber
		...
		{//already have it up, let's try to toggle it
			if ( cg_remoteClient )
			{	// coop: the host owns our playerState; it toggles the blade (and
				// the sounds) and we see it through the snapshot (g_cmds.cpp)
				cgi_SendClientCommand( "sabertoggle" );
			}
			else if ( !in_camera )
